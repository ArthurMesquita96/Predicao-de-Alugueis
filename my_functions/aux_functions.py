...
def plot_matrix(data, columns_features, n_rows, n_cols, plot, sort_by= None, plot_kwargs = {}, loop_feature = None, figsize = (15,15), label = True):
    ...
    for r in range(0, n_rows):
        for c in range(0, n_cols ):
            if (c + r*n_cols) >= len(columns_features):
                ...
                if sort_by:
                    data = data.sort_values(f'{sort_by}',ascending = True)
                else:
                    data = data.sort_values(f'{feature}',ascending = True)
...
def prepare_fit_and_predict(model, data_train, data_validation, dict_preparation, features_selected, log_on = True):
...
def cross_validation(data, dict_models, k_fold, len_train, len_validation, dict_preparation, features_selected, verbose=False):
    ...
    for model_name, model in dict_models.items():
        ...
        for k in range(1, k_fold+1):
            ...
            data_train_cv, data_validation_cv = split_dataset(data, k_fold, k, len_train, len_validation, verbose)
...
def split_dataset(data, k_fold, k, len_train, len_validation, verbose = False):
    '''
    len VALIDATION = 3
